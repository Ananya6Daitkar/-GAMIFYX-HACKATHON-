...
export const SkillChart: React.FC<SkillChartProps> = ({ data }) => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-900 border border-magenta-500/50 rounded-lg p-3 backdrop-blur-md">
          <p className="text-magenta-400 text-sm font-semibold">{data.language}</p>
          <p className="text-gray-300 text-sm">
            Proficiency: <span className="text-magenta-400">{data.proficiency}%</span>
          </p>
        </div>
      )
    }
            ...
            <BarChart data={data} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
              <defs>
                <linearGradient id="colorProficiency" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={THEME.colors.secondary} stopOpacity={0.8} />
                  <stop offset="95%" stopColor={THEME.colors.secondary} stopOpacity={0.3} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke={THEME.colors.secondary + '20'} />
              <XAxis
                dataKey="language"
